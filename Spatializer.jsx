...
export function Spatializer({ children, onOpenSource }) {
  ...
  const openExport = async () => {
    ...
    for (const scope of scopes) {
      try {
        results.push({ status: "fulfilled", value: await renderPngPreview(cameraRef.current, scope) });
      } catch (reason) {
